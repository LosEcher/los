...
test('chat keeps per-run choices beside the composer and evidence in the inspector', () => {
  ...
  assert.match(composer, /Run choices/);
  assert.match(composer, /Provider setup stays in Providers/);
  ...
  assert.match(composer, /advanced request settings/);
...
test('composer run controls are responsive instead of fixed to one crowded grid', () => {
  const controlGrid = between(styles, '.composer-control-grid {', '}');
  const denseGrid = between(styles, '.composer-control-grid.dense {', '}');

  assert.match(controlGrid, /repeat\(auto-fit, minmax\(150px, 1fr\)\)/);
  assert.match(denseGrid, /repeat\(auto-fit, minmax\(128px, 1fr\)\)/);
  assert.match(styles, /@media \(max-width: 780px\)[\s\S]*\.composer-control-grid,\n\s+\.composer-control-grid\.dense,\n\s+\.two-col\s+\{\n\s+grid-template-columns: 1fr/);
});
