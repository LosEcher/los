...
async function handleOnboarding(_req: FastifyRequest, _reply: FastifyReply) {
...
async function handleListCompatEvidence(req: FastifyRequest, _reply: FastifyReply) {
...
async function handleListPromotionDecisions(req: FastifyRequest, _reply: FastifyReply) {
  const query = req.query as { provider?: string; model?: string; limit?: string };
...
async function handleRecordPromotionDecision(req: FastifyRequest, reply: FastifyReply) {
...
async function handleEnforcePromotionDecision(req: FastifyRequest, reply: FastifyReply) {
  if (!(await requireOperator(req, reply))) return;
  const operator = getOperatorPrincipal(req);
  const body = asRecord(req.body);
  try {
...
async function handleListExternalSummaries(req: FastifyRequest, _reply: FastifyReply) {
...
async function handleScenarioEconomicsCorpus(_req: FastifyRequest, _reply: FastifyReply) {
  return getDailyAgentScenarioCorpus();
}

async function handleSummarizeScenarioEconomics(req: FastifyRequest, reply: FastifyReply) {
...
async function handleRecordScenarioEconomics(req: FastifyRequest, reply: FastifyReply) {
  if (!(await requireOperator(req, reply))) return;
  const body = asRecord(req.body);
...
async function handleImportExternalSummary(req: FastifyRequest, reply: FastifyReply) {
  try {
...
async function handleRunEvalSummary(req: FastifyRequest, reply: FastifyReply) {
...
async function handleRunEvalCompare(req: FastifyRequest, reply: FastifyReply) {
...
async function handleListRunEvals(req: FastifyRequest, _reply: FastifyReply) {
  const query = parseRunEvalQuery(req.query as RunEvalQuery);
  const evals = await listRunEvals(query);
  return { count: evals.length, evals };
}

async function handleListPairwise(req: FastifyRequest, _reply: FastifyReply, dependencies: ProviderEvidenceRouteDependencies) {
...
async function handleGetPairwise(req: FastifyRequest, reply: FastifyReply, dependencies: ProviderEvidenceRouteDependencies) {
...
async function handleRecordPairwise(req: FastifyRequest, reply: FastifyReply, dependencies: ProviderEvidenceRouteDependencies) {
  if (!(await requireOperator(req, reply))) return;
  const body = asRecord(req.body);
...
async function handleRecordRunEval(req: FastifyRequest, reply: FastifyReply) {
  const body = asRecord(req.body);
  const success = parseOptionalBoolean(body.success);
  if (success === undefined) return reply.status(422).send({ error: 'success is required' });
...
async function handleRecordEvalBacklog(_req: FastifyRequest, reply: FastifyReply) {
  try {
...
async function handleListEvalBacklog(_req: FastifyRequest, _reply: FastifyReply) {
...
export function registerProviderEvidenceRoutes(
  ...
  app.get('/onboarding', (req, reply) => handleOnboarding(req, reply));
  app.get('/providers/compat-evidence', (req, reply) => handleListCompatEvidence(req, reply));
  app.get('/providers/promotion-decisions', (req, reply) => handleListPromotionDecisions(req, reply));
  app.post('/providers/promotion-decisions', (req, reply) => handleRecordPromotionDecision(req, reply));
  app.post('/providers/promotion-decisions/enforce', (req, reply) => handleEnforcePromotionDecision(req, reply));
  app.get('/external-summaries', (req, reply) => handleListExternalSummaries(req, reply));
  app.get('/run-evals/scenario-economics/corpus', (req, reply) => handleScenarioEconomicsCorpus(req, reply));
  app.get('/run-evals/scenario-economics', (req, reply) => handleSummarizeScenarioEconomics(req, reply));
  app.post('/run-evals/scenario-economics', (req, reply) => handleRecordScenarioEconomics(req, reply));
  app.post('/external-summaries', (req, reply) => handleImportExternalSummary(req, reply));
  app.get('/run-evals/summary', (req, reply) => handleRunEvalSummary(req, reply));
  app.get('/run-evals/compare', (req, reply) => handleRunEvalCompare(req, reply));
  app.get('/run-evals', (req, reply) => handleListRunEvals(req, reply));
  app.get('/run-evals/pairwise', (req, reply) => handleListPairwise(req, reply, dependencies));
  app.get('/run-evals/pairwise/:pairId', (req, reply) => handleGetPairwise(req, reply, dependencies));
  app.post('/run-evals/pairwise', (req, reply) => handleRecordPairwise(req, reply, dependencies));
  app.post('/run-evals', (req, reply) => handleRecordRunEval(req, reply));
  app.post('/eval-backlog/run', (req, reply) => handleRecordEvalBacklog(req, reply));
  app.get('/eval-backlog', (req, reply) => handleListEvalBacklog(req, reply));
}
