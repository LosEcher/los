...
import {
  resolveProjectOwner,
  type ProjectOwnerResolution,
  type ResolveProjectOwnerInput,
} from '@los/agent/task-intake';
...
export function resolveConfiguredProjectOwner(
  input: Omit<ResolveProjectOwnerInput, 'bindings'>,
): ProjectOwnerResolution {
  return resolveProjectOwner({ ...input, bindings: listProjects() });
}
