...
function renderSweep(result: GovernanceSweepResult, json: boolean): void {
  ...
  for (const r of result.results) {
    ...
    if (r.jobType === 'consistency_audit') {
      ...
      console.log(`    detached=${summary.detached ?? '?'} dirty=${summary.workingTreeDirty ?? '?'} primary=${summary.primaryRemote ?? 'origin'} stale=${summary.stalePrimaryBranches ?? summary.staleOriginBranches ?? summary.staleCandidateCount ?? '?'} mirror=${summary.mirrorRemote ?? 'legacy'}:${summary.mirrorDrift ?? summary.forgejoDrift ?? '?'}(behind=${summary.mirrorBehind ?? summary.forgejoBehind ?? '?'} ahead=${summary.mirrorAhead ?? summary.forgejoAhead ?? '?'})`);
