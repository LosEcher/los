-- 044_scheduled_work_items.sql
-- User schedule definitions and per-slot execution evidence.
...
CREATE TABLE IF NOT EXISTS scheduled_work_item_runs (
  ...
  work_item_id TEXT REFERENCES todos(id) ON DELETE SET NULL,
  run_spec_id TEXT,
  task_run_id TEXT,
...
CREATE INDEX IF NOT EXISTS idx_scheduled_work_due
  ON scheduled_work_items(status, circuit_state, next_run_at);
CREATE INDEX IF NOT EXISTS idx_scheduled_work_project
  ON scheduled_work_items(project_id, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_scheduled_work_runs_schedule
  ON scheduled_work_item_runs(schedule_id, scheduled_for DESC);
CREATE INDEX IF NOT EXISTS idx_scheduled_work_runs_lease
  ON scheduled_work_item_runs(status, lease_expires_at);
