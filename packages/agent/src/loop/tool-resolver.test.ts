...
test('empty allowedTools array behaves like undefined (not a zero-tool allowlist)', () => {
  ...
  assert.ok(!readOnly.includes('spawn_agent'), 'read-only mode cannot spawn children');
  assert.ok(readOnly.includes('query_agent'), 'control-plane query stays visible in read-only mode');
