...
export const DAILY_GAP_TODO_SEED: CreateTodoInput[] = [
...

  {
    ...
    metadata: {
      ...
      closedNote: '2026-08-07: four-factor decay model implemented + documented in packages/memory/src/core/decay.ts (baseScore x recency x referenceCount x toolStatus, STALE_THRESHOLD 0.3); boundary tests in decay.test.ts',
  ...
  },
  {
    ...
    metadata: {
      ...
      closedNote: '2026-08-07: calculateDecayScores(sessionId) in decay.ts returns per-observation scores + session aggregates (averageScore/staleRatio/staleObservationIds); excludes archived AND compacted observations (new-observation semantics, fixed 2026-08-07); DB tests in decay.test.ts',
      candidateFiles: ['packages/memory/src/core/decay.ts'],
    },
  },
  {
    ...
    metadata: {
      ...
      closedNote: '2026-08-07: shouldTriggerCompaction(sessionId) in decay.ts implements both rules (low_decay: averageScore<0.3 && obs>=20; high_stale: staleRatio>0.4); auto_trigger persisted via compactSession input (memory_compactions.auto_trigger); 24h safety net lives in gateway server-maintenance; tests in decay.test.ts',
      candidateFiles: ['packages/memory/src/core/decay.ts'],
    },
  },
  {
    ...
    metadata: {
      ...
      closedNote: '2026-08-07: gateway/server-maintenance.ts runMemoryMaintenance wired to decay trigger + 24h safety net (autoTrigger decay|scheduled, force=true). Fixed 2026-08-07: candidate query had mc.id IS NULL (dead — every session had event-driven compaction rows) and compactSession dedup blocked re-compaction; selectAutoCompactCandidates now selects sessions with non-archived non-compacted observations; tests in server-maintenance.test.ts',
  ...
  },
  {
    ...
    metadata: {
      ...
      closedNote: '2026-08-07: aggregateCrossSessionDecay(excludeSessionId) in decay.ts groups kinds across sessions with decayRate estimation; wired into compactSession Phase 5 (packages/memory/src/core/compaction.ts) producing cross_session_decay observed patterns + procedural candidates',
      candidateFiles: ['packages/memory/src/core/compaction.ts'],
    },
  },
];
