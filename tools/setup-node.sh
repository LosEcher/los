...
#   LOS_REPO_URL=git@github.com:echerlos/los.git   # Only for first-time git clone
...
install_deps() {
  ...
  if $LOW_RESOURCE; then
    su - "$LOS_USER" -c "cd $LOS_HOME && NODE_OPTIONS='--max-old-space-size=128' pnpm install --frozen-lockfile --no-optional --workspace-concurrency=1" || {
      warn "pnpm install failed in low-resource mode"
      warn "Check memory: free -h && swapon --show"
      return 1
    }
  else
    su - "$LOS_USER" -c "cd $LOS_HOME && pnpm install --frozen-lockfile" || {
...
log ""
log "=== Setup complete ==="
log "Executor running at http://127.0.0.1:${EXECUTOR_PORT:-8090}"
