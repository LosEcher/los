...
| Provider compatibility | Compatibility evidence exists and `los provider promote` remains setup-only; required-gate promotion still needs a separate policy command | Can run checks externally | Login/readiness can be misleading | Not provider authority | Not provider authority | Define provider promotion/demotion policy commands only when ADR and harness updates can land together |
...
1. expose run contract fields in CLI/UI after verifier and recovery entrypoints can consume
   them;
2. define provider promotion/demotion policy commands for required-target changes;
